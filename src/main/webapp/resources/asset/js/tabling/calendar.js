(function ($) {
  ...
  function generateWeekDOM(monthData, currentDate) {
    ...
    monthData.forEach(function (week, weekNo) {
      ...
      week.forEach(function (day, dayNo) {
        ...
        today = today ? " today" : "";
		//console.log("디스에이블" + settings.disable, day);
        ...
        if (
          ...
          //dateDisabled = 'abled="no" ';
          abled = ' no';
        }
        
        let dataStr = "";
        //console.log(settings.myDisabledDates);
        
        let tyear = day.getFullYear();
		let tmonth = day.getMonth() + 1;
		let tday = day.getDate();
        
        let tmp = settings.myDisabledDates.find(d => (d[0] === tyear && d[1] === tmonth && d[2] === tday));
        const fullBooking = "";
        if(tmp){
        	console.log("같다냥", day, tmp);
        	dataStr += ' fullb="fullb" ';
        	//dateDisabled = 'abled="no" ';
        	fullbk = ' fullbk';
        }
  ...
  function highlightWeek() {
    let all = el.find(".day[disabled!='disabled']");
    for(let i = 7; i < all.length; i++){
      all.eq(i).addClass("highlightc");
    }
    // let last = el.find("div.day[disabled='disabled']")[disable_len-1];
    // console.log("오눌", el.find(".day[disabled!='disabled']").length, el.find(".day[disabled!='disabled']"));
    // el.find(".selected").parents(".week").addClass("highlight");
  }

  function highlightWeek2() {
    el.find(".selected").parents(".week").addClass("highlightj");
  }
  ...
  $.fn.calendar = function (options) {
    ...
    if(settings.myDisabledDates) {
      //console.log("my~" + settings.myDisabledDates);
      settings.myDisabledDates = settings.myDisabledDates;
    }
    ...
    el.off("click", ".day").on("click", ".day", function (e) {
      ...
      if($(this).hasClass('today')){ //오늘클릭
      	settings.onClickDate(date);
      	return;
      }
      
       if ($(this).hasClass("fullbk")){ //예약이 마감된 날
       //alert('예약마감일 클릭');
       settings.onClickNOT('예약이 마감되었습니다.');
       return "예약마감일 클릭";
       }
      
      if ($(this).hasClass('no')){
       //alert('안되는 거 클릭');
       settings.onClickNOT('예약불가한 날입니다.');
       return "안되는 거 클릭";
       }
